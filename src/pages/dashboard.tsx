...
import { getSalesDetails } from 'apiSdk/sales-details';
...
import { SalesDetailListPage as ShowCaseList } from 'pages/sales-details';

function DashboardPage() {
  const { session } = useSession();

  const salesDetailStat = useSWR<PaginatedInterface<SalesDetailInterface>>(
    () => `/sales-details`,
    () => getSalesDetails({ limit: 1, offset: 0 }),
  );

  const salesinvoiceStat = useSWR<PaginatedInterface<SalesinvoiceInterface>>(
    () => `/salesinvoices`,
    () => getSalesinvoices({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Sales Details',
      result: salesDetailStat?.data?.totalCount,
      isLoading: salesDetailStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Salesinvoices',
      result: salesinvoiceStat?.data?.totalCount,
      isLoading: salesinvoiceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`sales-details`} passHref legacyBehavior>
                  <Button
